...
export class ClientConversationManager {
  ...
  updateWithChatResponse(
    ...
    if (messageIds.length > 0) {
      ...
      const assistantMessageId =
        assistantIds.length > 0 ? Math.max(...assistantIds) : undefined;
  ...
  getAllConversations(): ClientConversation[] {
    ...
    return list.sort((a, b) => {
      ...
      const aTime = a.backendConversation?.updatedAt
        ? new Date(a.backendConversation.updatedAt).getTime()
        : 0;
      const bTime = b.backendConversation?.updatedAt
        ? new Date(b.backendConversation.updatedAt).getTime()
        : 0;
  ...
  loadBackendConversations(backendConversations: Conversation[]): void {
    const backendConversationIds = new Set(
      backendConversations.map((c) => c.id),
    );

    for (const [conversationId] of this.conversations.entries()) {
      if (
        !backendConversationIds.has(conversationId) &&
  ...
  handleExternalCreate(conversation: Conversation): void {
    ...
    if (Object.keys(normalizedConversation.messages).length > 0) {
      clientConv.messages = this.buildMessagesFromBackend(
