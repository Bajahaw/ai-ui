...
export const SettingsDataProvider = ({ children }: { children: ReactNode }) => {
  ...
  const refreshProviderModelsFn = useCallback(
    async (id: string) => {
  ...
  const deleteMCPServer = useCallback(
    async (id: string) => {
      ...
      setData((d) => ({
        ...d,
        mcpServers: d.mcpServers.filter((s) => s.id !== id),
      }));
  ...
  const refreshMCPToolsFn = useCallback(
    async (id: string) => {
      await refreshMCPToolsApi(id);
      // Refresh tools and MCP servers to sync with updated DB state
      await refreshTools();
