...
func GetBuiltInTools() []*Tool {
	return []*Tool{
		...
		},
		{
			ID:              uuid.New().String(),
			Name:            "http_request",
			Description:     "HTTPS request to a public host (DNS names only; no IPs; private/loopback/metadata blocked). Response bodies are reduced by default (HTML scrub, base64/data-URI omit, text cap). Binary bodies omitted (metadata+SHA256). Use verbose only when you need the raw payload. Secrets: $secrets.NAME$ in headers or URL path/query only — not host or body.",
			InputSchema:     `{"type":"object","properties":{"url":{"type":"string","description":"Full https URL with a DNS hostname (not an IP). $secrets.NAME$ allowed in path and query."},"method":{"type":"string","description":"GET, HEAD, POST, PUT, PATCH, or DELETE","default":"GET"},"headers":{"type":"object","additionalProperties":{"type":"string"},"description":"Optional request headers. $secrets.NAME$ allowed in values. Host/Transfer-Encoding etc. rejected."},"body":{"type":"string","description":"Optional body for POST/PUT/PATCH/DELETE (max 1MiB). No secret placeholders."},"verbose":{"type":"boolean","description":"If true, skip body reduction. Use only when you need the raw payload.","default":false}},"required":["url"]}`,
