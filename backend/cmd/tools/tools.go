...
func GetBuiltInTools() []*Tool {
	return []*Tool{
		...
		},
		{
			ID:              uuid.New().String(),
			Name:            "http_request",
			Description:     "HTTPS request to a public host (DNS names only; no IPs; private/loopback/metadata blocked). Response bodies are reduced by default (HTML scrub, base64/data-URI omit, 32KiB text cap). Binary bodies omitted (metadata+SHA256). verbose skips reduction and raises the text cap to 256KiB. For JSON responses, pass json_pointers (RFC 6901) to return only selected values instead of the full body. Secrets: $secrets.NAME$ in headers or URL path/query only — not host or body.",
			InputSchema:     `{"type":"object","properties":{"url":{"type":"string","description":"Full https URL with a DNS hostname (not an IP). $secrets.NAME$ allowed in path and query."},"method":{"type":"string","description":"GET, HEAD, POST, PUT, PATCH, or DELETE","default":"GET"},"headers":{"type":"object","additionalProperties":{"type":"string"},"description":"Optional request headers. $secrets.NAME$ allowed in values. Host/Transfer-Encoding etc. rejected."},"body":{"type":"string","description":"Optional body for POST/PUT/PATCH/DELETE (max 1MiB). No secret placeholders."},"verbose":{"type":"boolean","description":"If true, skip body reduction and raise text output cap from 32KiB to 256KiB. Use only when you need the raw/larger payload.","default":false},"json_pointers":{"type":"array","items":{"type":"string"},"description":"Optional RFC 6901 JSON Pointers into a JSON response body (e.g. \"/data/items/0/id\"). Returns a JSON object keyed by each pointer. Empty string is the document root. Requires JSON body; missing paths error."}},"required":["url"]}`,
